...
public class ActivitiServiceImpl implements ActivitiService {
	...
	public boolean devlopActiviti(String folderPath, HashMap<String, String> map) {
		DeploymentBuilder deploymentBuilder = repositoryService.createDeployment()// 创建一个部署构建器
				...
				.name(map.get("name")).category(map.get("category"));
		if (map.get("tenantId") != null) {
			// 多租房的概念
			deploymentBuilder.tenantId(map.get("tenantId"));
		}
	...
	public boolean getTaskByLoginName(String processDefinitionKey, String loginName) {
		...
		// 任务列表
		// List<Task> list =
		// taskQuery.processDefinitionKey(processDefinitionKey).taskAssignee(role.getRoleCode())
		...
		for (String resourceName : resourceNames) {
			if (resourceName.endsWith(".png")) {
				...
				try {
					...
					// 把流写入到文件中
					String pathName = "C:\\Users\\xzzz\\Desktop\\diagrams\\" + resourceName;
